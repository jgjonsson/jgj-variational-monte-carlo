...
cout << "This round " << count << " of PRE-TRAINING gets " << 10000 << " iterations per thread" << endl;
...
#pragma omp parallel shared(pretrainSamplers, count)
    {
        ...
        auto system = std::make_unique<PretrainSystem>(
            ...
            std::make_unique<TwoLayerNetworkWavefunction>(rbs_M, rbs_N, params, omega, alpha, beta, 0.0),
...
////// Ok, lets try get some pre-training going
    ...
    //Keep training old file if supplied, otherwise start train from new random parameters.
    //auto parametersInputFile = argc > 9 ? argv[9] : "";
    if(!(parametersInputFile && parametersInputFile[0] != '\0'))
    {
        cout << "Randomizing parameters." << endl;
        params = NeuralNetworkTwoLayers::generateRandomParameterSetTwoLayers(rbs_M, rbs_N, parameter_seed, parameterGuessSpread);
    }
    else
    {
    ...
    int max_iterations_pre_training = 200;
    /*
    if(parametersInputFile && parametersInputFile[0] != '\0')
    ...
    else*/
    {
        ...
        string paramFilename = "NNparams_pretrain_" + fileSuffix  + "_twolayer.csv";
        string kFilename = "K_pretrain_" + fileSuffix + "_twolayer.csv";
